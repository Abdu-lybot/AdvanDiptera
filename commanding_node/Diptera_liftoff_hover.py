...
class Lift_Off:
        ...
        if (self.flying_status == "Drone_lifted"):
            while (rospy.is_shutdown() is False) and (self.flying_status != "LANDING"):
                if (self.hover_hight - 0.03 <= self.local_pose.pose.position.z <= self.hover_hight + 0.03):
                    ...
                    Basic_movement().move_to_z(self.hover_hight - self.local_pose.pose.position.z)
    ...
    def start(self):
        ...
        while self.local_pose.pose.position.z < (self.takeoff_height + self.threshold) and self.down_sensor_distance < (self.takeoff_height + self.threshold):
            self.autotakeoff()
            time.sleep(0.2)
